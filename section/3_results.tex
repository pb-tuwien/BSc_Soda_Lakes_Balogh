...
which alignes with the $4.2-8.6\,\mu\text{s}$, found by \textcite{aignerFlexibleSingleLoop2021}.
...
which should be a bit smaller for smaller loop sizes~\parencite{aignerFlexibleSingleLoop2021}.
The long lasting effect of the turn-off ramp for the soundings in the water, 
can on one side be attributed to a longer turn-off ramp in water~\parencite{aignerFlexibleSingleLoop2021} and can
...
The inversion algorithm~\parencite{aignerFlexibleSingleLoop2021} used for the inversion needs various starting parameters.
...
\textcite{welkensComparisonSubsurfaceGeophysical2025} shows that model with $1\,\text{m}$-Layers until $5\,\text{m}$ depth 
...
Regarding the noise floor, the minimum value for the relative error used in the inversion, \textcite{aignerSensitivityAnalysisInverted2024}
...
measurement configuration resembles the configuration with the smaller loop by \textcite{aignerSensitivityAnalysisInverted2024}.
...
This range covers the lambda values \textcite{aignerSensitivityAnalysisInverted2024} used for the inversion 
...
described by \textcite{hansenAnalysisDiscreteIllPosed1992,farquharsonComparisonAutomaticTechniques2004}, but TEM data
...
and thus being discarded in the first iteration of the search algorithm. \textcite{cultreraSimpleAlgorithmFind2020} described 
