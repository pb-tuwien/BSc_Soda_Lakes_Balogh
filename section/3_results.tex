...
and start increasing for all soundings for time gates later than $30\,\mu\text{s}$.
...
Thus all the time gates outside the time range $8-110\,\mu\text{s}$ were filtered, as shown in \cref{fig:good-signal-oct}(c) and (d).
...
show effects of the turn-off ramp until $12\,\mu\text{s}$ (\cref{fig:err-signal-oct}(b)).
...
For the first survey of this thesis a $12.5\times12.5\,\text{m}$ loop was used 
and \cref{fig:inv-depth-10m}(c) shows that the inversion algorithm struggles to fit the late time data, 
...
and thus the relative root-mean-square (rRMS) of the misfit between modelled and observed data was used to quantify the imporvement,
...
which was computed with the parameters from \cref{subsec:inversion-parameters}:
...
The remaining $42$ soundings of the first survey were manually analysed 
...
the RMS misfit increases between lambdas $5-8$ and decreases between $63-100$, 
but the curve still has the characteristic ``corner'' at the $\lambda$ value of $8$,
...
It is plausible that this behaviour changes with further increases in the $\lambda$ values (outside of the investigated range), 
...
where the L-Curve method produced no cohesive curve, but rather collection of points with seemingly random positions.
...
but that study used a different loop size and a diffent inversion algorithm, 
...
We also computed the means of the relative deviations between the visually obtained $\lambda_\text{opt}$ 
...
\cref{tab:search-summary} shows that the Gradient Based approach had the lowest
...
In this section we compare the inversion results computed for the different measurement configurations 
...
Ideally this comparison would be done with soundings collected at the same time as well as the same location.
...
and a current of $1.0\,\text{A}$ (``large loop and small current''), and
