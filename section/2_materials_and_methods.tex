...
we conducted a field survey in October of 2024 at the Martenhofer Lacke in the \textit{Nationalpark Neusiedlersee - Seewinkel}. 
...
By connecting an external $12~\text{or}~24\,V$ battery a current either $1~\text{or}~4\,A$ can be put through the connected transmitter loop~\parencite{aignerFlexibleSingleLoop2021}.
...
where $n_s$ is the chosen stacking-key ($1-20$) and $n_{as}$ is the number of analogue stacks depending on the chosen time-key 
...
The relative error limits how strongly the inversion algorithm tries to fit the model to each data point to avoid creating artifacts.
