...
which is called log-gating~\parencite{christiansen2006tem}.
...
and thus later time gates are linked to information about deeper layers~\parencite{christiansen2006tem}.
The depth of investigation (DOI) is limited by the electrical conductivity of the subsurface,
because a EM wave attenuates stronger within a conductive layer~\parencite{everett2013tem}.
For this reason the TEM method is commonly deployed to investigate conductive layers such as sea water intrusions, and
clay layers confining aquifers~\parencite{chandra2016tem}.

Every geophysical measurement includes uncertainty
and a single transient is usually significantly influenced by background noise~\parencite{christiansen2006tem,everett2013tem}.
To increase the S/N ratio of a TEM sounding measurents are repeated 
and the resulting transients are combined, which is called ``stacking''~\parencite{christiansen2006tem}.
Commonly between $1000$ and $10000$ transients are stacked into one sounding~\parencite{aigner2021flexible} 
...
measurements per stack~\parencite{christiansen2006tem}.
...
which equals to the weighted mean of the resistivities of the layers in which the currents were induced~\parencite{siemon2006rhoa}.
...
$\mu_0$ is the magnetic permeability of free space and $t$ is the time~\parencite{fitterman1986transient}.
...
infered through the apparent resistivities in (c), which makes the interpretation of the data necessary~\parencite{fitterman1986transient,christiansen2006tem}.
Reconstructing the resistivity distribution of the subsurface requires the solution of ill-posed problem, 
as there are many subsurface models, which can explain the measured data~\parencite{xue2020inversion}.
...
as most inversion algorithms work with a one dimensional model, the subsurface is assumed to be homogeneous in the horizontal direction~\parencite{christiansen2006tem}.
...
To provide an optimal signal-to-noise ratio the device automatically stacks multiple pulses~\parencite{barsukov2015temfast}.
...
leading to its high salinity and shallow water depth, which also varies throughout the year~\parencite{boros2025seewinkel}.
...
which combines the electromagnetic wave modelling capabilities of \texttt{empymod}~\parencite{werthmüller2017empymod}
...
This makes the choice of an appropriate layer distribution (specifying the number and thicknesses of layers) vital~\parencite{welkens2025seewinkel}.
...
corresponding to each inversion, which should result in an L-Curve~\parencite{cultrera2020lcurve, hansen1992lcurve}.

To find an optimal lambda value for the inversion we implemented several search algorithms, 
which all try to find the point (corresponding to a lambda value) with the highest curvature on the L-Curve~\parencite{lloyd1997lcurve,cultrera2020lcurve}.
We implemented the method used by \textcite{lloyd1997lcurve}, which fits a cubic spline function to the L-Curve and 
...
Lastly we implemented the iterative golden section search algorithm as described by \textcite{cultrera2020lcurve}, 
