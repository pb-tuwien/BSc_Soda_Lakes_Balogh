...
\end{equation}
where $M$ is the magnetic moment, $\frac{\partial b_z}{\partial t}$ is the time derivative of the vertical magnetic field,
$\mu_0$ is the magnetic permeability of free space and $t$ is the time.


The method operates by transmitting a pulse of current through a loop, generating a primary magnetic field.
When the current is abruptly turned off, it induces secondary currents (eddy currents) in the ground,
which decay over time and produce a secondary electromagnetic field.
This field is measured using a receiver coil,
and its decay characteristics are analyzed to infer the conductivity structure of the subsurface.
TEM is particularly effective for groundwater exploration and hydrogeological mapping,
as it provides high-resolution data on aquifers, geological structures, and cover materials.
Compared to frequency-domain methods, TEM is advantageous because it eliminates the need to measure the primary field directly,
allowing for deeper penetration and improved sensitivity to low-resistivity layers such as clay and water-bearing formations.
However, TEM requires sophisticated electronics and advanced computational techniques for data processing and interpretation.
