...
Electrical resistivity is a key parameter in geophysical investigations, as it provides valuable information about the Earth's subsurface properties.
...
By analyzing these resistivity values, it is possible to infer the composition of subsurface layers
...
By analyzing the decay characteristics of the secondary field, it is possible to infer the conductivity structure of the subsurface,
...
As the interpretation of the data is computationally intensive and requires sophisticated electronics to measure the secondary field accurately,
...
This can be done by using the formula for late stage behavior of the secondary field:
\begin{equation}
...
and the slope of the curve shows, if the resistivity is increasing or decreasing in the new layer~\parencite{fitterman1986transient}.
...
For the interpretation of TEM data, and for the inversion in particular, some assumptions have to be made.
This EM wave is assumed to only propagate vertically and the subsurface is assumed to be layered.
As most inversion algorithms are based on a 1D model, the subsurface is assumed to be homogeneous in the horizontal direction.
...
Galvanic coupling is caused by grounded conductors like power lines and causes the underestimation of the resistivity.
...
For this reason the study recommends to keep a distance of about $150\,\text{m}$ from underground cables or pipes when the earth has a resistivity of $40-50\,\Omega\text{m}$.
...
Similar to the PATEM system, \textcite{auken2019ttem} propose a towed Transient Electromagnetic (tTEM) system for an efficient 3D mapping of the subsurface.
The tTEM system utilizes a $2\times4\,\text{m}$ transmitter loop mounted on a non-conductive sled towed by an ATV, which enables a production rate of about $1\,km^2$ per day.
The receiver loop is towed at an $9\,\text{m}$ offset from the transmitter.
Like the PATEM system, the tTEM system allows two different currents ($2.8~\text{and}~30\,\text{A}$) in order to gain a relatively high DOI of up to $70\,\text{m}$,
while still allowing the investigation of shallow depths.
This study also highlights the importance of considering coupling effects of the system with conductive objects
and tested the system in an environment with a high resistivity of the subsurface ($>600\,\Omega\text{m}$).
In this environment the signal caused by coupling can be observed in isolation
and the study found that a minimal distance of $3\,\text{m}$ between transmitter and ATV was necessary.
...
Electromagnetic methods do not require direct contact with the subsurface, which allowed the development of airborne electromagnetic methods (AEM).
\textcite{sorensen2004skytem} introduced the SkyTEM system, as alternative to ground based TEM systems.
...
The SkyTEM system was validated against a ground based TEM system with a transmitter loop size of $40\times40\,\text{m}$ showed a good agreement (below 5\% deviation).
This system is able to cover a larger area than traditional ground based systems in the same time and was still able to resolve underground structures,
...
However, the receiver loops are deployed in an offset configuration and
for the $0.5\times0.5\,\text{m}$ loop only a current of $2\,\text{A}$ was used, while the whole $18\,\text{A}$ was applied for the $10\times10\,\text{m}$ loop.
...
Solving inverse problems is a task not limited to geophysics, which makes inversion theory an important field of mathematics.
