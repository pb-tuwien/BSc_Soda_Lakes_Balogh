The TEM method was originlly introduced for mining applications~\parencite{chandra_time_2016} 
and was since adopted to other fields like groundwater, 
and geothermal exploration~\parencite{chandra_time_2016, everett_electromagnetic_2013}.
The transient electromagnetic (TEM) or altenatively time-domain electromagnetic (TDEM) method was
proposed by \textcite{obukhov_fields_1968} and has been intensively developed 
in the 1980s~\parencite{christiansen_transient_2006}.
...
generating a primary magnetic field~\parencite{chandra_time_2016,christiansen_transient_2006}.
When the current is turned off, the decay of the primary field induces 
secondary currents (eddy currents) in the ground,
which produce a secondary magnetic field~\parencite{chandra_time_2016,christiansen_transient_2006}.
This is governed by the Maxwell equations, which state that a changing electrical field 
induces a magnetic field, and vice versa~\parencite{christiansen_transient_2006}.
An electromagnetic (EM) wave propagating through the subsurface, 
is attenuated by the electrical properties of the materials it encounters~\parencite{christiansen_transient_2006,everett_electromagnetic_2013}.
...
information about the subsurface can be obtained~\parencite{christiansen_transient_2006,chandra_time_2016}.
...
which is called log-gating~\parencite{christiansen_transient_2006}.
...
and thus later time gates are linked to information about deeper layers~\parencite{christiansen_transient_2006}.
The depth of investigation (DOI) is limited by the electrical conductivity of the subsurface,
because a EM wave attenuates stronger within a conductive layer~\parencite{everett_electromagnetic_2013}.
For this reason the TEM method is commonly deployed to investigate conductive layers such as sea water intrusions, and
clay layers confining aquifers~\parencite{chandra_time_2016}.

Every geophysical measurement includes uncertainty
and a single transient is usually significantly influenced by background noise~\parencite{christiansen_transient_2006,everett_electromagnetic_2013}.
To increase the S/N ratio of a TEM sounding measurents are repeated 
and the resulting transients are combined, which is called ``stacking''~\parencite{christiansen_transient_2006}.
Commonly between $1000$ and $10000$ transients are stacked into one sounding~\parencite{aigner_flexible_2021} 
...
measurements per stack~\parencite{christiansen_transient_2006}.
...
which equals to the weighted mean of the resistivities of the layers in which the currents were induced~\parencite{siemon_electromagnetic_2009}.
...
$\mu_0$ is the magnetic permeability of free space and $t$ is the time~\parencite{fitterman_transient_1986}.
...
infered through the apparent resistivities in (c), which makes the interpretation of the data necessary~\parencite{fitterman_transient_1986,christiansen_transient_2006}.
Reconstructing the resistivity distribution of the subsurface requires the solution of ill-posed problem, 
as there are many subsurface models, which can explain the measured data~\parencite{xue_development_2020}.
...
as most inversion algorithms work with a one dimensional model, the subsurface is assumed to be homogeneous in the horizontal direction~\parencite{christiansen_transient_2006}.
...
To provide an optimal signal-to-noise ratio the device automatically stacks multiple pulses~\parencite{barsukov_chapter_2006}.
The number of stacks are given by the formula $P_{tot} = 13 \times n_s \times n_{as}$~\parencite{aigner_flexible_2021}, 
...
leading to its high salinity and shallow water depth, which also varies throughout the year~\parencite{boros_how_2025}.
This location was chosen due to having sparse man-made structures in the Nationalpark, 
which reduces noise in the gathered data to a minimum~\parencite{aigner_sensitivity_2024}.
...
For the inversion routine we built upon the work of \textcite{aigner_flexible_2021}, 
which combines the electromagnetic wave modelling capabilities of \texttt{empymod}~\parencite{werthmuller_open-source_2017}
with the inversion algorithm from \texttt{PyGIMLi}~\parencite{rucker_pygimli_2017}.
...
This makes the choice of an appropriate layer distribution (specifying the number and thicknesses of layers) vital~\parencite{welkens_comparison_2025}.
...
corresponding to each inversion, which should result in an L-Curve~\parencite{cultrera_simple_2020, hansen_analysis_1992}.

To find an optimal lambda value for the inversion we implemented several search algorithms, 
which all try to find the point (corresponding to a lambda value) with the highest curvature on the L-Curve~\parencite{lloyd_use_1997,cultrera_simple_2020}.
We implemented the method used by \textcite{lloyd_use_1997}, which fits a cubic spline function to the L-Curve and 
...
Lastly we implemented the iterative golden section search algorithm as described by \textcite{cultrera_simple_2020}, 
...
% second survey loop in water, not halfspace