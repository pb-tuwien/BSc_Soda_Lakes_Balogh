...
in a field survey in the soda lakes of the Nationalpark Neusiedlersee - Seewinkel and
...
This was then visually interpreted to find an optimal $\lambda$, which worked for $83.33\,\%$ of the sounding in the first,
...
could be used as the $\lambda$ for the inversions instead of computing the optimal value for each sounding seperately, 
while for the second survey this technique did not produce adequate results.

These manually determined $\lambda$ values were then used to compare three search algorithms used to automatically find the 
optimal $\lambda$ by search for the point with the maximum curvature. 
...
Our research indicates that the current of $4.1\,\text{A}$ can be used to resolve the first three layers,
...
and numerical modelling to characterise, under which conditions the L-Curve technique is applicable for the analysis of TEM data 
